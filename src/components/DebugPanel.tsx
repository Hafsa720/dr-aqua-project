...
const DebugPanel: React.FC<DebugPanelProps> = ({
  ...
  const getBreakpointInfo = () => {
    ...
    if (windowSize.width < 640)
      return { name: 'Mobile', icon: Smartphone, color: 'text-red-500' };
    if (windowSize.width < 768)
      return { name: 'SM', icon: Tablet, color: 'text-orange-500' };
    if (windowSize.width < 1024)
      return { name: 'MD', icon: Tablet, color: 'text-yellow-500' };
    if (windowSize.width < 1280)
      return { name: 'LG', icon: Monitor, color: 'text-green-500' };
