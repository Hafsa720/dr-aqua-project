import { servicesEn, testimonialsEn, whyChooseUsEn, serviceLabelsEn } from './en';
import { servicesUr, testimonialsUr, whyChooseUsUr, serviceLabelsUr } from './ur';
...
export const getServiceById = (id: string, language: string): Service | undefined => {
  const services = getServices(language);
  return services.find((s) => s.id === id);
};

export const getServiceBySlug = (slug: string, language: string): Service | undefined => {
  const services = getServices(language);
